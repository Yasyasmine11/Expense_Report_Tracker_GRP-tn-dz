        ...
        APIGatewayProxyRequest request, ILambdaContext context)
    {
        try
        {
            ...
            if (!IsFinanceManager(claims))
                return Forbidden("Only Finance Managers can reject expense reports.");

            var body      = JsonConvert.DeserializeObject<RejectDto>(request.Body);
            var expenseId = request.PathParameters["expenseId"];

            if (body == null || string.IsNullOrEmpty(body.UserId))
                return BadRequest("UserId is required.");

            // La justification est OBLIGATOIRE pour un rejet
            if (string.IsNullOrWhiteSpace(body.Justification))
                return BadRequest("Justification is required when rejecting an expense.");
            ...
            {
                return BadRequest(
                    $"Cannot reject an expense with status '{currentStatus}'. " +
            ...
            await _dynamo.UpdateItemAsync(new UpdateItemRequest
            {
                ...
                UpdateExpression =
                    "SET #s = :s, StatusGSI_PK = :gsi, justification = :j, " +
                    "reviewedBy = :mgr, updatedAt = :ts",
                ExpressionAttributeNames  = new() { ["#s"] = "status" },
                ExpressionAttributeValues = new()
                {
                    [":s"]   = new() { S = "REJECTED" },
                    [":gsi"] = new() { S = "STATUS#REJECTED" },
                    [":j"]   = new() { S = body.Justification },
                    [":mgr"] = new() { S = managerId },
            ...
            context.Logger.LogInformation(
                $"Expense {expenseId} REJECTED by manager {managerId}: {body.Justification}");

            return Ok("{\"message\":\"Expense rejected\",\"status\":\"REJECTED\"}");
        }
        catch (Exception ex)
        {
            context.Logger.LogError($"Error rejecting expense: {ex.Message}");
